...
class BrightfieldGeneratorCpProcessor extends modObjectProcessor {
    public $id = null;
    public $data = null;
    ...
    public function getHTML($id = 0, $type = 'kits') {
        ...
        foreach($this->data[$type] as $id=>$obj) {
            ...
            if(!empty($packs['html'])) {
                $page .= $this->modx->getChunk('order.list.thead', array(
        ...
        foreach ($this->data['packs'] as $id=>$pack) {
            if($pack['kitid'] == $kitid) {
                ...
                if(!empty($products['html'])) {
                    ...
                    if($pack['title']) {
                        $result['html'].= $this->modx->getChunk('order.list.title', $pack);
                    }
    ...
    public function getProducts($packid) {
        ...
        foreach ($this->data['products'] as $product) {
            if($product['pack_id'] == $packid) {
                $result['html'].= $this->modx->getChunk('order.list.row', $product);
...
return 'BrightfieldGeneratorCpProcessor';